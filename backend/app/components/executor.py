...
from app.contracts import BUY_LIKE_ACTIONS, NAV_FIELDS, POSITION_FIELDS, SELL_LIKE_ACTIONS, SIM_FILL_FIELDS
...
def run(ctx: Any, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        sim_fill, positions, nav = _execute_trade_plan(ctx, payload, trade_plan)
        executor_failed = False
        stage_label = "simulated_execution_cn_a_share"
    except Exception as exc:
        sim_fill, positions, nav = _fallback_execution(ctx, payload, trade_plan, str(exc))
        executor_failed = True
        stage_label = "executor_fallback"

    write_csv(ctx.artifact_path("sim_fill_t.csv"), sim_fill, SIM_FILL_FIELDS)
    write_csv(ctx.artifact_path("positions_t.csv"), positions, POSITION_FIELDS)
    write_csv(ctx.artifact_path("nav_t.csv"), nav, NAV_FIELDS)

    next_payload = dict(payload)
    stage_notes = dict(payload.get("stage_notes", {}))
    filled_order_count = len([row for row in sim_fill if row["status"] == "FILLED"])
    total_fees = sum(to_float(row.get("total_fee")) for row in sim_fill)
    stage_notes["executor"] = f"{stage_label}; filled_orders={filled_order_count}; fees={total_fees:.2f}"

    next_payload["sim_fill"] = sim_fill
    next_payload["positions"] = positions
    next_payload["nav"] = nav
    next_payload["executor_failed"] = executor_failed
...
def _execute_trade_plan(
    ...
    positions_prev = [dict(item) for item in payload.get("positions_prev", [])]
    ...
    watchlist_by_symbol = {
        ...
        for item in payload.get("snapshot", {}).get("watchlist", [])
    }
    ...
    for plan_row in trade_plan:
        ...
        sim_fill.append(
            {
                "trade_date": ctx.trade_date,
                "order_id": plan_row.get("order_id", ""),
                "symbol": symbol,
...
def _fallback_execution(
    ...
    positions = [dict(item) for item in payload.get("positions_prev", [])]
    account = dict(payload.get("account", {}))
    sim_fill = [
        {
            "trade_date": ctx.trade_date,
...
def _finalize_positions(
    *,
        ...
        positions.append(
            {
                ...
                "board": position.get("board", market_view.get("board", "UNKNOWN")),
                "quantity": quantity,
    ...
    return sorted(positions, key=lambda item: str(item.get("symbol", "")))
