...
from app.contracts import BUY_LIKE_ACTIONS, SELL_LIKE_ACTIONS, TRADE_PLAN_FIELDS
from app.utils import index_by, to_bool, to_float


def run(ctx: Any, payload: dict[str, Any]) -> dict[str, Any]:
    orders_candidate = list(payload.get("orders_candidate", []))
    try:
        trade_plan, risk_events = _build_trade_plan(ctx, payload, orders_candidate)
    ...
    write_csv(ctx.artifact_path("trade_plan_t.csv"), trade_plan, TRADE_PLAN_FIELDS)

    next_payload = dict(payload)
    stage_notes = dict(payload.get("stage_notes", {}))
    stage_notes["risk_guard"] = (
    ...
    next_payload["trade_plan"] = trade_plan
    next_payload["risk_events"] = risk_events
    next_payload["risk_guard_failed"] = risk_guard_failed
...
def _build_trade_plan(
    ...
    tech_candidates_by_symbol = index_by(list(payload.get("tech_candidates", [])), "symbol")
    ...
    ordered_candidates = sorted(orders_candidate, key=lambda row: action_priority.get(str(row.get("action", "")), 99))
