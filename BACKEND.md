...
- 配置文件可通过 `pipeline.preset` 或 `pipeline.stages` 选择阶段集合
- CLI 可用 `--pipeline-preset` / `--pipeline-stages` 临时覆写
- API 可通过 `pipeline_preset` / `pipeline_stages` 在请求级别覆写
- stage 业务逻辑不再直接负责写盘，CSV / JSON / Markdown 导出由编排层统一收口
...
可通过接口触发单日任务、读取持仓、交易计划、成交、NAV 和报告，也支持调用东方财富自然语言选股和资讯搜索。
...
运行后端前请先安装依赖。`PyYAML` 用于加载配置，Wind MySQL 相关依赖也包含在这里：
...
export WIND_DB_PASS=ELPWN2YJRXBCQKYd
...
```bash
export DB_WIND_URL='mysql+pymysql://wind_admin:***@10.100.0.28:3306/winddb?charset=utf8mb4'
```
...
```text
examples/input/daily_snapshot.json
```
...
```text
outputs/demo-single/
outputs/backtests/backtest-demo-123456/
```
...
- `full`：完整 7 段
- `holding_review`：只做持仓复核
- `research`：`selector + analyst`
- `decision`：到 `decider` 为止
- `planning`：到 `risk_guard` 为止
- `execution`：`executor + reporter`，适合对已存在 `trade_plan` 做后处理

也可以直接写阶段列表：

```text
pipeline:
  stages: update_holding_actions, selector, analyst
```

运行时会做依赖校验，例如直接跑 `decider` 但未提供 `holding_actions` / `ai_insights` 时，会在编排层直接报错。
...
| `stages/*.json` | 每个阶段的紧凑调试快照：关键输入、stage 输出增量、artifact manifest |
...
python3 run_api.py \
  ...
  --port 8787 \
  --output-root outputs \
  --default-config app/config/pipeline.yaml \
  --default-input examples/input/daily_snapshot.json
```

完整接口说明见 `docs/api-mvp.md`。
...
| `selection.source` | `snapshot` 或 `stock_screen` |
...
| `stock_screen.default_page_size` | 东方财富选股单页大小 |
| `stock_screen.max_pages` | 东方财富选股最大翻页数 |
| `news_search.default_size` | 东方财富资讯搜索默认条数 |
| `llm.enable_live` | 是否启用 live LLM |
| `degrade.disable_selector` | 关闭 selector 做降级测试 |
| `degrade.disable_executor` | 关闭 executor 做降级测试 |

### 7.3 Selector 使用东方财富选股

如果希望 `selector` 阶段直接调用东方财富自然语言选股，可在配置中设置：
...
- `selector` 候选来自东方财富 API
- AI 分析阶段仍继续使用 Wind 技术面数据
- 需要设置环境变量 `MX_APIKEY`
...
python3 run_single_day.py --run-id live-llm-demo
```

当前 `update_holding_actions`、`analyst`、`decider`、`reporter` 依赖 live LLM。
...
- 功能架构：`docs/architecture.md`
- API 调用：`docs/api-mvp.md`
- 产品规划：`docs/backend-plan.md`
